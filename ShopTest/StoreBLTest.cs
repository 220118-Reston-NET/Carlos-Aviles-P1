...
namespace ShopTest
{
    ...
    public class StoreBLTest
    {
        ...
        public void StoreShouldUpdate()
        {
            ...
            Mock<IStoreRepo> mockRepo = new Mock<IStoreRepo>();
            mockRepo.Setup(repo => repo.UpdateStore(_Store)).Returns(_Store);

            IStores Stores = new Stores(mockRepo.Object);

            //act
            StoreFront actual = Stores.UpdateStore(_Store);

            //assert
            Assert.Same(_Store, actual);
        }

        [Fact]
        public void StoreShouldGet()
        {
            //arrange
            int id = 1;
            string name = "Nutrient North";
        ...
        [Fact]
        public async Task StoreShouldGetAsync()
        {
            //arrange
            int id = 1;
            string name = "Nutrient North";
            ...
            Mock<IStoreRepo> mockRepo = new Mock<IStoreRepo>();
            mockRepo.Setup(repo => repo.GetStoresAsync()).ReturnsAsync(expectedList);

            IStores Stores = new Stores(mockRepo.Object);

            //act
            List<StoreFront> actualList = await Stores.GetStoresAsync();
